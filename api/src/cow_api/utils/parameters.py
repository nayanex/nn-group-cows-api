...
class CowKey:
    def __init__(
        self,
        name: str = Query(..., description="Name of the cow. The input will be converted to lower case."),
    ):
        self.name = name
...
class CowParam:
    ...
    def initialize(self, session: Session) -> None:
        self.cow = db_models.Cow.find_by_functional_key(self.cow_key.name, session).first()
